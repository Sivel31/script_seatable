...
VMSELECT_URL = "https://pdp-user-vmauth.data.corp/api/v1/query"
TOKEN = os.getenv("VICTORIA_PDP_TOKEN")
if not TOKEN:
    logger.error("Environment variable VICTORIA_PDP_TOKEN is not set")
    raise ValueError("Environment variable VICTORIA_PDP_TOKEN is not set")
TIMEOUT = 10
...
def get_all_software_versions(time_range="3h"):
    ...
    for project_id in VM_PROJECT_IDS:
        main_query = f'last_over_time({{job="script_exporter_sys_app_versions", __name__=~"filebeat_version|consulmdm_version", vm_project_id="{project_id}"}}[{time_range}])'
    ...
    # 1. Запрос для vmagent (vm_app_version)
    for project_id in VM_PROJECT_IDS:
...
def sync_versions_to_seatable():
    ...
    for key, vm_data in vm_software_versions.items():
        ...
        if vm_host.lower().startswith('mdm'):
            continue
        ...
        if key in existing_entries:
            ...
            if version_changed or contour_changed:
                ...
                if version_changed:
                    ...
                    changes_log.append(f"Версия: {current_version if current_version else 'пусто'} -> {vm_version}")

                if contour_changed:
                    updates["Контур"] = vm_contour
                    changes_log.append(f"Контур: {current_contour if current_contour else 'пусто'} -> {vm_contour}")
    ...
    if rows_to_create:
        ...
        for new_row in rows_to_create[:10]:
            logger.info(f"  Хост: {new_row['host']}, Контур: {new_row['contour']}, ПО: {new_row['software']}, Версия: {new_row['version']}")
    ...
    if rows_to_update:
        ...
        for update in rows_to_update:
            try:
                ...
                updated_count += 1
                logger.debug(f"Обновлен хост {update['host']}, ПО: {update['software']}")
            except Exception as e:
                logger.error(f"Ошибка при обновлении хоста {update['host']}, ПО {update['software']}: {e}")
    ...
    if rows_to_create:
        ...
        for new_row in rows_to_create:
            try:
                ...
                created_count += 1
                logger.debug(f"Создан хост {new_row['host']}, Контур: {new_row['contour']}, ПО: {new_row['software']}")
            except Exception as e:
                logger.error(f"Ошибка при создании записи для хоста {new_row['host']}: {e}")
...
def main():
    ...
    logger.info("="*60)
    logger.info(f"Начало синхронизации версий ПО: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info("="*60)

#############################################################################################
# Скрипт получает метрики из VictoriaMetrics ПДП                                           #
...
logging.basicConfig(
    ...
    handlers=[
        RotatingFileHandler(
            "pdp-update.log", maxBytes=10 * 1024 * 1024, backupCount=5, encoding="utf-8"
        ),
...
# Конфигурация VictoriaMetrics для PDP
...
def sync_cpu_ram_mountpoints_to_seatable():
    """Синхронизирует данные ТОЛЬКО для хостов, начинающихся с 'pdp'."""
    ...
    # Фильтруем строки из SeaTable: оставляем ТОЛЬКО хосты, начинающиеся с 'pdp'
    host_rows = []
    for row in rows:
        host_val = row.get("Host")
        if host_val is not None and str(host_val).strip():
            host_key = str(host_val).strip()
            # ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ В ОТБОРЕ: работаем только с хостами pdp
...
def main():
    """Запускает процесс синхронизации для хостов PDP."""
    logger.info("=" * 60)
    logger.info(
        f"Начало синхронизации для PDP хостов: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    logger.info("=" * 60)
